...
def log_activity(db: Session, user_id: Optional[int], action: str, details: dict = None):
    ...
    try:
        ...
        db.add(log_entry)
        db.commit()
        print(f"✓ Logged activity: {action} for user {user_id}")
...
def load_system_prompts():
    ...
    try:
        ...
        print(f"[OK] Loaded {len(SYSTEM_PROMPTS)} system prompts from file")
        for key in SYSTEM_PROMPTS:
            print(f"  - {key}: {len(SYSTEM_PROMPTS[key])} chars")
            
    except Exception as e:
        print(f"[ERROR] Failed to load system prompts: {e}")
...
async def call_openai(system_prompt: str, user_message: str, temperature: float = 0.7, max_tokens: int = None) -> str:
    ...
    try:
        if not openai.api_key:
            print("⚠️  No OpenAI API key configured, using mock screening")
        ...
        try:
            ...
            print(f"⚠️  Fallback API also failed: {e2}")
            print("Using mock screening response instead")
            return generate_mock_screening_response()
...
async def screen_candidate(
    ...
    try:
        ...
        if not system_prompt:
            raise HTTPException(status_code=500, detail="Screening prompt not loaded")
...
async def bulk_screen(
    ...
    try:
        ...
        if not system_prompt:
            raise HTTPException(status_code=500, detail="Bulk screening prompt not loaded")
...
"""
        ...
        if not response:
            print("⚠️  Empty response from OpenAI, using mock screening")
            response = generate_mock_screening_response()
        
        print(f"API Response (first 300 chars): {response[:300]}...")
        ...
        try:
            ...
            if results_data is None and parse_error:
                try:  
                    ...
                    print(f"JSON parse debug: response len={len(clean_response)}, first 200 chars: {clean_response[:200]}")
            ...
            print(f"Backend: Expected {len(request.candidates)} candidates, got {len(results_data)} results")
            if len(results_data) < len(request.candidates):
                print(f"WARNING: Not all candidates were screened! Expected {len(request.candidates)}, got {len(results_data)}")
                print(f"Generating mock results for missing candidates...")
                ...
                for candidate in request.candidates:
                    ...
                    if candidate_name not in screened_names:
                        print(f"  - Generating mock result for {candidate_name}")
                        mock_result = {
            ...
            # Add timestamp to each result and ensure all fields are set
            ...
            try:
                ...
                db.commit()
                print(f"✅ Successfully saved {saved_count}/{len(results_data)} screening results to database")
            ...
            print(f"⚠️  Using mock screening response as fallback")
            response = generate_mock_screening_response()
            ...
            if results_data:
                try:
                    ...
                    print(f"Warning: Could not save to DB: {db_error}")
                    db.rollback()
        ...
        print(f"✗ Bulk screening error: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
...
#TechJobs #Hiring #{request.job_title.replace(' ', '')} #RemoteWork #Innovation"""
        ...
        print(f"✗ Job post generation error: {e}")
        
        # Final fallback: Generate basic template posts if all else fails
        print("Using final fallback template generation...")
        fallback_posts = {
            "client_project": "NA",
            "recruitment_type": "Permanent",
            "role": request.job_title,
...
async def upload_bulk_resumes(
    ...
    try:
        ...
        print(f"📥 Received {len(files)} files for bulk processing")
        ...
        for file in files:
            try:
                ...
                print(f"Processing file: {filename} ({len(file_content)} bytes, ext: {ext})")
        ...
        print(f"Final result: {len(candidates)} candidates, {len(processing_errors)} errors")
    ...
    try:
        ...
        if not logs:
            ...
            if log_file.exists():
                ...
                if logs:
                    print(f"✅ Loaded {len(logs)} logs from file")
...
async def get_screening_results(current_user: Optional[User] = Depends(get_optional_user), db: Session = Depends(get_db)):
    ...
    print("[SCREENING-RESULTS] Endpoint called")
    try:
        # Use the same ORM query that works in /api/logs endpoint
        print("[SCREENING-RESULTS] Querying database...")
        screening_results = db.query(ScreeningResult).order_by(ScreeningResult.created_at.desc()).limit(50).all()
        print(f"[SCREENING-RESULTS] Query returned {len(screening_results)} results")
        ...
        print(f"[SCREENING-RESULTS] Returning {len(results)} results")
        return {"results": results}
    except Exception as e:
        print(f"[SCREENING-RESULTS] Error: {e}")
        import traceback
        traceback.print_exc()
